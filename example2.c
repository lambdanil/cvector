...
int main() {
    ...
    while (1) {
        if (scanf("%d",&current) == 1) {
            ...
            added = 0;
            for (int i = 0; i < (int)numbers.size; i++) added+=(vector_at(&numbers,i));
            average = (float) added / (int)numbers.size;
            printf("Average of entered numbers is %0.3f\n",average);
        }
        else {
            printf("Average of numbers:");
            for (int i = 0; i < (int)numbers.size-1; i++) printf(" %d,", vector_at(&numbers,i));
            printf(" %d", vector_at(&numbers, (int)numbers.size-1));
            average = (float) added / (int)numbers.size;
            printf(" is %0.3f \n",average);
            break;
        }
